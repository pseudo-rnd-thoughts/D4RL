...
import numpy as np
from pybullet_envs import env_bases, scene_abstract
...
class MazeRobot(bullet_robot.MJCFBasedRobot):
    def __init__(self, maze_spec):
        ...
        filename = os.path.join(
            offline_env.DATASET_PATH, "tmp_bullet_xml", maze_hash + ".xml"
        )
...
class Maze2DBulletEnv(env_bases.MJCFBaseBulletEnv, offline_env.OfflineEnv):
    def __init__(self, maze_spec, reward_type="dense", reset_target=False, **kwargs):
        ...
        # Set the default goal (overridden by a call to set_target)
    ...
    def create_single_player_scene(self, bullet_client):
        return scene_abstract.SingleRobotEmptyScene(
    ...
    def reset(self):
        ...
        env_bases.MJCFBaseBulletEnv.reset(self)
        if self.stateId < 0:
    ...
    def reset_model(self):
        ...
        qpos = reset_location + self.np_random.uniform(low=-0.1, high=0.1, size=2)
        qvel = self.np_random.randn(2) * 0.1
