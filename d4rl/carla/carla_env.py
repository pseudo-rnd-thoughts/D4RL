...
from d4rl.carla.data_collection_town import get_font

# from . import proxy_env
from d4rl.offline_env import OfflineEnv
from d4rl.utils.wrappers import ProxyEnv

try:
    sys.path.append(
        glob.glob(
            f"../carla/dist/carla-*{sys.version_info.major:d}.{sys.version_info.minor:d}-{'win-amd64' if os.name == 'nt' else 'linux-x86_64'}.egg"
        )[0]
    )
except IndexError:
    pass
...
class CustomGlobalRoutePlanner(GlobalRoutePlanner):
    ...
    def compute_direction_velocities(self, origin, velocity, destination):
        node_list = super()._path_search(origin=origin, destination=destination)
    ...
    def compute_distance(self, origin, destination):
        ...
        for idx in range(len(node_list) - 1):
            distances.append(
                super()._distance_heuristic(node_list[idx], node_list[idx + 1])
            )
...
class Storm:
    ...
    def __str__(self):
        return (
            f"Storm(clouds={self.clouds:d}%, rain={self.rain:d}%, wind={self.wind:d}%)"
        )
...
class CarlaEnv:
    ...
    def goal_reaching_reward(self, vehicle):
        ...
        try:
            self.route_planner.compute_distance(vehicle_location, target_location)
    ...
    def lane_follow_reward(self, vehicle):
        ...
        if goal_waypoint is None:
            print(
                f"Episode fail: goal waypoint is off the road! (frame {self.count:d})"
            )
            done, vel_s = True, 0.0
        else:
            ...
            if len(next_goal_waypoint) == 0:
                print(f"Episode done: no more waypoints left. (frame {self.count:d})")
        ...
        # not algorithm's fault, but the simulator sometimes throws the car in the air weirdly
    ...
    def _simulator_step(self, action, traffic_light_color):
        ...
        if self.render_display:
            self.render_display.blit(
                self.font.render(f"Frame {self.count:d}", True, (255, 255, 255)),
                (8, 10),
            )
            self.render_display.blit(
                self.font.render(
                    f"Control: {throttle:5.2f} throttle, {steer:5.2f} steer, {brake:5.2f} brake",
                    True,
                    (255, 255, 255),
                ),
                (8, 28),
            ...
            self.render_display.blit(
                self.font.render(
                    f"Traffic light: {traffic_light_color}", True, (255, 255, 255)
                ),
                (8, 46),
            )
            self.render_display.blit(
        ...
        if self.render_display and self.record_display:
            image_name = os.path.join(self.record_dir, f"display{self.count:08d}.jpg")
        ...
        if self.record_vision:
            image_name = os.path.join(self.record_dir, f"vision{self.count:08d}.png")
            print("savedimg:", image_name)
            im = Image.fromarray(rgb)

            # add any metadata you like into the image before we save it:
            metadata = PngInfo()
            ...
            #     metadata.add_text(f"reward_{str(key)}", str(reward_dict[key]))
            #
            # for key in done_dict:
            #     metadata.add_text(f"done_{str(key)}", str(done_dict[key]))
    ...
    def finish(self):
        ...
        for actor in self.actor_list:
            actor.destroy()
        print(f"\ndestroying {len(self.vehicles_list):d} vehicles")
        self.client.apply_batch(
            [carla.command.DestroyActor(x) for x in self.vehicles_list]
        )
        time.sleep(0.5)
