...
from d4rl.locomotion import goal_reaching_env, maze_env
...
class GoalReachingPointDictEnv(goal_reaching_env.GoalReachingDictEnv, PointEnv):
    """Ant locomotion for goal reaching in a dictionary compatible format."""
...
def create_maze_navigation_policy(maze_env):
    ...
    goal_reaching_policy = create_goal_reaching_policy(
        obs_to_ori=lambda obs: obs[ori_index]
    )

    return maze_env.create_navigation_policy(
        lambda obs, goal: goal_reaching_policy(np.concatenate([obs, goal]))
    )
